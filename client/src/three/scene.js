...
export function createRiftScene(container, { onPortalChange, onInteract } = {}) {
    ...
    function playEmote(group, emoteName) {
        ...
        group.userData.activeAction?.fadeOut(0.1);
        group.userData.activeAction = action;

        const durationMs = (action.getClip().duration || 1.1) * 1000;
        group.userData.emoteUntil = performance.now() + durationMs;
    }
    ...
    function onKeyDown(e) {
        ...
        if ((k === '1' || k === '2') && !e.repeat && localAvatar) {
            const emoteName = k === '1' ? 'emote-yes' : 'emote-no';
            playEmote(localAvatar, emoteName);
            emoteCallback?.(emoteName);
        }
    }
    ...
    function playRemoteEmote(callsign, emoteName) {
        const entry = remotes.get(callsign);
        if (entry) playEmote(entry.group, emoteName);
    }
    ...
    function tick() {
        ...
        if (localAvatar) {
            ...
            const airborne = localY > 0.02;
            if (performance.now() > (localAvatar.userData.emoteUntil || 0)) {
                setAvatarMotion(
                    localAvatar,
                    airborne
                        ? (verticalVelocity > 0 ? 'jump' : 'fall')
                        : (speed > 0.35 ? 'walk' : 'idle')
                );
            }
        ...
        for (
            ...
            const remoteAirborne = entry.target.y > 0.02;
            const remoteRising = remoteAirborne && entry.target.y > (entry.lastY ?? 0);
            entry.lastY = entry.target.y;

            if (performance.now() > (entry.group.userData.emoteUntil || 0)) {
                setAvatarMotion(
                    entry.group,
                    remoteAirborne
                        ? (remoteRising ? 'jump' : 'fall')
                        : (performance.now() < entry.movingUntil
                            ? 'walk'
                            : 'idle')
                );
            }
